...
- **Lazy ticks**: the market is anchored to a wall-clock epoch. Any page load or API call advances the simulation to "now" (capped at 15 days per request). A CAS guard on `market_state.market_day` makes concurrent serverless invocations safe — each day is simulated exactly once. An optional hourly Vercel cron (`/api/cron/tick`) keeps the world moving during quiet hours.
